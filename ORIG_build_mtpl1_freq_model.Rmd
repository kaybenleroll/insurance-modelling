...
```{r construct_prior_setup_table, echo=TRUE}
...
```{r construct_prior_first_model_system, echo=TRUE, cache=TRUE}
...
We now plot for `freq_mean` for each of the sample policies in our dataset.

```{r plot_priorpred1_pois_output_sample, echo=TRUE}
prior_freqmodel1_freqmean_plot_tbl <- priorpred1_pois_data_tbl %>%
  select(prior_id, a_s, prior_output) %>%
  unnest(prior_output)

ggplot(prior_freqmodel1_freqmean_plot_tbl %>% filter(a_s == FALSE)) +
...
ggplot(prior_freqmodel1_freqmean_plot_tbl %>% filter(a_s == TRUE)) +
  geom_boxplot(aes(x = prior_id, group = prior_id, y = freq_mean)) +
  scale_y_log10() +
  xlab("Prior Params ID") +
  ylab("Mean of Claim Frequency") +
...
ggplot(prior_freqmodel1_maxcount_plot_tbl %>% filter(a_s == FALSE)) +
  geom_histogram(aes(x = max_count), bins = 50) +
  scale_x_log10(labels = label_comma()) +
  facet_wrap(vars(prior_id), nrow = 4, scales = "free") +
  xlab("Max Claim Count") +
  ylab("Frequency") +
  ggtitle("Histograms of Max Claim Count by Sets of Prior Parameters (Non-scaled)") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))


ggplot(prior_freqmodel1_maxcount_plot_tbl %>% filter(a_s == TRUE)) +
  geom_histogram(aes(x = max_count), bins = 50) +
  scale_x_log10(labels = label_comma()) +
  facet_wrap(vars(prior_id), nrow = 4, scales = "free") +
  xlab("Max Claim Count") +
  ylab("Frequency") +
  ggtitle("Histograms of Max Claim Count by Sets of Prior Parameters (Scaled)") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
```
...
```{r inspect_priorpred1_pois_output, echo=TRUE}
calc_max_count <- ~ .x %>%
  summarise(
    .groups = "drop",
...
maxcount_tbl <- priorpred1_pois_data_tbl %>%
  mutate(
    max_count_data = map(prior_output, calc_max_count)
    ) %>%
  select(-prior_params, -prior_output) %>%
...
```{r write_free_priorpred1_pois_data, echo=FALSE, results="hide"}
priorpred1_pois_data_tbl %>% write_rds("data/priorpred1_pois_data_tbl.rds")
```
...
than a Poisson model for the counting process.

```{r calculate_priorpred1_negbin, echo=TRUE, cache=TRUE}
...
```{r plot_priorpred1_negbin_output_sample, echo=TRUE}
prior_freqmodel1_freqmean_plot_tbl <- priorpred1_negbin_data_tbl %>%
  select(prior_id, a_s, prior_output) %>%
  unnest(prior_output)
...
```{r inspect_priorpred1_negbin_output, echo=TRUE}
maxcount_tbl <- priorpred1_negbin_data_tbl %>%
  mutate(
    max_count_data = map(prior_output, calc_max_count)
    ) %>%
  select(-prior_params, -prior_output) %>%
...
```{r write_free_priorpred1_negbin_data, echo=FALSE}
priorpred1_negbin_data_tbl %>% write_rds("data/priorpred1_negbin_data_tbl.rds")
```
...
fam_params_tbl <- tibble(fam_str = c("poisson", "neg_binomial_2")) %>%
  mutate(data = list(priorpred_priorparams_tbl)) %>%
  select(data, fam_str) %>%
  unnest(data) %>%
  mutate(
    prior_id    = 1:n(),
    dist_family = map(fam_str, ~ .x %>% get() %>% exec())
    )


priorpred2_data_tbl <- fam_params_tbl %>%
  mutate(
    prior_lst = pmap(
        list(prior_mean  = p_m,
             prior_sd    = p_s,
...
prior_freqmodel2_freqmean_plot_tbl <- priorpred2_data_tbl %>%
  select(prior_id, fam_str, a_s, prior_output) %>%
  unnest(prior_output) %>%
  arrange(prior_id, fam_str, a_s, policy_id, .draw)
...
maxcount_priorpred2_tbl <- priorpred2_data_tbl %>%
  mutate(
    max_count_data = map(prior_output, calc_max_count)
    ) %>%
  select(-dist_family, -prior_params, -prior_output) %>%
  unnest(max_count_data)

maxcount_priorpred2_tbl %>% datatable()
```
...
We now have some priors for our two models to work with.
...
We now turn our attention to fitting a Bayesian model using our training data
and we will evaluate these models using both in-sample and out-of-sample data.
