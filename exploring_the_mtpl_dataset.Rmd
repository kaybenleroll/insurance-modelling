---
title: "Exploring the MTPL Dataset"
...
gcs_auth("credentials/google-application-credentials.json")
...
For data security reasons, we do not save this file to disk, but instead we
upload the data to Google Cloud Storage.

We do not want all the data provided in the SSLA for the moment, instead we
keep a subset of the data, for the purposes of building a frequency and
severity model.

```{r subset_columns, echo=TRUE}
output_data_tbl <- shrink_ssla_data(data_tbl)

output_data_tbl %>% glimpse()
```

Having constructed the output data we now upload the data to GCS.

```{r upload_to_gcs, echo=TRUE}
f <- function(input, output) write_rds(input, path = output)

output_data_tbl %>%
  gcs_upload(name            = "ssla_output_tbl.rds",
             bucket          = "sslasecure",
...
facet_varname <- "case_status_simple"
