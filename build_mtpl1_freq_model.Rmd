---
title: "MTPL1: Build the First Frequency Model"
...
mtpl1_training_tbl <- mtpl1_split %>% training() %>% arrange(as.numeric(policy_id))
mtpl1_training_tbl %>% glimpse()

mtpl1_testing_tbl  <- mtpl1_split %>% testing()  %>% arrange(as.numeric(policy_id))
mtpl1_testing_tbl %>% glimpse()
```

Finally, we want a small set of policies from both the training and testing
datasets (about 20 from each), to use as a consistent sample of the data to
use for illustrative purposes.

```{r sample_smaller_data_subsets, echo=TRUE}
n_small_sample <-  25
n_sample       <- 250
...
priorpred_input_tbl <- fit_data_tbl %>%
  ...
  arrange(as.numeric(policy_id))

priorpred_input_tbl %>% glimpse()


mtpl1_training_small_tbl <- priorpred_input_tbl %>%
  slice_sample(n = n_small_sample) %>%
  arrange(as.numeric(policy_id))

mtpl1_testing_small_tbl  <- mtpl1_testing_tbl %>%
  slice_sample(n = n_small_sample) %>%
  arrange(as.numeric(policy_id))
```
...
our frequency model, we create a simple summary function
`calculate_mean_freq_summary` that calculates this summary information in a
standard way.
...
```{r freq1_calculate_prior_predict_policy_summaries, echo=TRUE}
...
```{r freq2_calculate_prior_predict_summaries, echo=TRUE}
...
```{r freq2_mean_freq_boxplots, echo=TRUE}
...
prior_freqmodel1_freqmean_pois_plot_tbl <- priorpred1_pois_data_tbl %>%
  select(prior_id, a_s, freqmodel_output) %>%
  unnest(freqmodel_output)
...
ggplot(prior_freqmodel1_freqmean_pois_plot_tbl %>% filter(a_s == FALSE)) +
  ...
  geom_hline(aes(yintercept = ref_mean), colour = "green") +
  geom_hline(aes(yintercept = max_mean), colour = "red") +
...
From a visual inspect of the unscaled priors, there seems to be two good
candidates: priors 4 and 8.

Rather than choose, we will try both and check what effect the different priors
have.

The autoscaled priors we always get unfeasibly large possible claim counts, so
we are likely to exclude them, but just to try them out to see what effect
there is, we will use prior 11.

```{r show_selected_poisson_priors, echo=TRUE}
priorpred_poisson_tbl <- priorpred_priorparams_tbl %>%
  filter(prior_id %in% c(4, 8, 11))

priorpred_poisson_tbl %>% print()
```
...
For the negative binomial model, it looks like prior 22 is the one to use with
unscaled priors, and for the autoscaled we will try prior 9.


```{r show_selected_negbin_priors, echo=TRUE}
priorpred_negbin_tbl <- priorpred_priorparams_tbl %>%
  filter(prior_id %in% c(9, 22))

priorpred_negbin_tbl %>% print()
```
...
We now want to use our selected set of prior parameters with the data to see
what effect the various sets of priors have on our fitted models, and also to
see how our Bayesian regression models work to produce various frequency
models.


## Fitting the Poisson Model

We start by fitting the Poisson-based frequency model, using the previously
selected set of prior parameters for our model.

```{r calculate_fitted_model_poisson, echo=TRUE, cache=TRUE}
freqmodel_pois_data_tbl <- priorpred_priorparams_tbl %>%
  filter(prior_id %in% c(4, 8, 11)) %>%
  mutate(
    prior_lst = pmap(
        list(prior_mean = p_m,
...
freqmodel_pois_data_tbl %>% glimpse()
```

We now want to compare the prior and posterior parameter distributions.

```{r combine_freqmodel1_poisson_models, echo=TRUE}
prior_tbl <- priorpred1_pois_data_tbl %>%
  semi_join(freqmodel_pois_data_tbl, by = "prior_id")


freqmodel_pois_compare_tbl <- list(
    prior     = prior_tbl,
    posterior = freqmodel_pois_data_tbl
    ) %>%
  bind_rows(.id = "label") %>%
  arrange(prior_id)

freqmodel_pois_compare_tbl %>% glimpse()
```


We now want to investigate the effect of things like posterior shrinkage and
the effect of priors on the various policies, so we choose a sample of the
policies and then check those values.

```{r plot_sample_policy_frequencies, echo=TRUE}
plot_tbl <- freqmodel_pois_compare_tbl %>%
  select(prior_id, label, freqmodel_output) %>%
  unnest(freqmodel_output) %>%
  mutate(prior_id = sprintf("%02d", prior_id)) %>%
  filter(label == "posterior") %>%
  semi_join(mtpl1_training_small_tbl, by = "policy_id")

ggplot(plot_tbl) +
  geom_boxplot(aes(x = policy_id, y = freq_mean, colour = prior_id)) +
  expand_limits(y = 0) +
  labs(x = "Policy ID", y = "Mean Frequency", colour = "Prior") +
  ggtitle("Boxplot of Posterior Distribution of Sample Policies (Poisson)") +
  theme(axis.text.x = element_text(angle = 30, vjust = 0.5))
```


```{r write_freqmodel_pois_data_disk, echo=TRUE}
freqmodel_pois_compare_tbl %>% write_rds("data/freqmodel_pois_compare_tbl.rds")
```
...
```{r calculate_fitted_model_negbin, echo=TRUE, cache=TRUE}
freqmodel_negbin_data_tbl <- priorpred_priorparams_tbl %>%
  filter(prior_id %in% c(9, 22)) %>%
  mutate(
    prior_lst = pmap(
        list(prior_mean = p_m,
...
freqmodel_negbin_data_tbl %>% glimpse()
```


```{r combine_freqmodel1_negbin_models, echo=TRUE}
prior_tbl <- priorpred1_negbin_data_tbl %>%
  semi_join(freqmodel_negbin_data_tbl, by = "prior_id")


freqmodel_negbin_compare_tbl <- list(
    prior     = prior_tbl,
    posterior = freqmodel_negbin_data_tbl
    ) %>%
  bind_rows(.id = "label") %>%
  arrange(prior_id)

freqmodel_negbin_compare_tbl %>% glimpse()
```

```{r plot_sample_policy_frequencies_negbin, echo=TRUE}
plot_tbl <- freqmodel_negbin_compare_tbl %>%
  select(prior_id, label, freqmodel_output) %>%
  unnest(freqmodel_output) %>%
  mutate(prior_id = sprintf("%02d", prior_id)) %>%
  filter(label == "posterior") %>%
...
ggplot(plot_tbl) +
  ...
  expand_limits(y = 0) +
  labs(x = "Policy ID", y = "Mean Frequency", colour = "Prior") +
  ggtitle("Boxplot of Posterior Distribution of Sample Policies (Neg Binomial)") +
  theme(axis.text.x = element_text(angle = 30, vjust = 0.5))
```



```{r write_freqmodel_negbin_data_disk, echo=TRUE}
freqmodel_negbin_compare_tbl %>% write_rds("data/freqmodel_negbin_compare_tbl.rds")
```


## Compare Poisson and Negative Binomial Posteriors

```{r construct_comparison_tables, echo=TRUE}
distrib_comparison_tbl <- list(
    Poisson        = freqmodel_pois_compare_tbl,
    `Neg Binomial` = freqmodel_negbin_compare_tbl
    ) %>%
  bind_rows(.id = "distrib")

distrib_comparison_tbl %>% glimpse()
```


```{r compare_pois_negbin_posterior_parameters, echo=TRUE}
plot_tbl <- distrib_comparison_tbl %>%
  filter(
    label == "posterior"
    ) %>%
  select(distrib, label, freqmodel_params) %>%
  unnest(freqmodel_params) %>%
  pivot_longer(
    !c(distrib, label, .chain, .iteration, .draw),
    names_to  = "parameter",
    values_to = "value"
    ) %>%
  filter(str_detect(parameter, "__$", negate = TRUE)) %>%
...
ggplot(plot_tbl) +
  geom_boxplot(aes(x = parameter, y = value, colour = distrib)) +
  labs(x = "Parameter", y = "Value", colour = "Distribution") +
  ggtitle("Comparison of Posterior Distribution Parameters") +
  theme(axis.text.x = element_text(angle = 30, vjust = 0.5))
```


```{r compare_pois_negbin_posterior_meanfreq, echo=TRUE}
plot_tbl <- distrib_comparison_tbl %>%
  filter(
    label == "posterior"
    ) %>%
  select(distrib, label, freqmodel_output) %>%
  unnest(freqmodel_output) %>%
  semi_join(mtpl1_training_small_tbl, by = "policy_id")

ggplot(plot_tbl) +
  geom_boxplot(aes(x = policy_id, y = freq_mean, colour = distrib)) +
  expand_limits(y = 0) +
  labs(x = "Policy ID", y = "Frequency Mean", colour = "Distribution") +
  ggtitle("Comparison of Posterior Distribution of the Mean Frequency") +
  theme(axis.text.x = element_text(angle = 30, vjust = 0.5))
```

The parameters and the mean frequency is similar, so we also check the sample
counts produced by the `generated quantities` in the Stan model.

```{r plot_comparison_sample_count_barplots, echo=TRUE}
barplot_tbl <- plot_tbl %>%
  count(Distribution = distrib, policy_id, sample_count)

ggplot(barplot_tbl) +
  geom_col(aes(x = sample_count, y = n, fill = Distribution),
           width = 0.4, position = "dodge") +
  scale_y_continuous(labels = label_comma()) +
  facet_wrap(vars(policy_id), scales = "free") +
  xlab("Claim Count") +
  ylab("Frequency") +
  ggtitle("Comparison Barplots for Select Policies for Distributions")
```
