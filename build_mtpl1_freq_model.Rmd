...
calculate_priorpred_freqmean <- function(priorpred_stanreg,
                                         priorpred_input_tbl) {
...
priorpred1_data_tbl <- priorpred_priorparams_tbl %>%
  mutate(
    ...
    prior_freqmean = map(prior_stanreg,
                         calculate_priorpred_freqmean,
                         priorpred_input_tbl = priorpred_input_tbl)
    )

priorpred1_data_tbl %>% write_rds("data/priorpred1_data_tbl.rds", compress = "xz")

priorpred1_data_tbl %>% glimpse()
```

We now have quite a lot of data, so we take a sample of 10,000 rows of each
...
prior_freqmodel1_plot_tbl <- priorpred1_data_tbl %>%
  mutate(prior_sample = map(prior_freqmean, ~ .x %>% slice_sample(n = 10000))) %>%
...
From this it looks like the appropriate prior model is the first one: we centre
our intercept on -6, and use autoscaling.

This will be the prior we use once we are fitting our model.

```{r write_priorpred1_free, echo=FALSE, results="hide"}
...
priorpred2_data_tbl %>% write_rds("data/priorpred2_data_tbl.rds", compress = "xz")
...
```{r write_priorpred2_free, echo=FALSE, results="hide"}
