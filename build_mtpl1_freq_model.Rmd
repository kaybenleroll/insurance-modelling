...
mtpl1_training_20_tbl <- mtpl1_training_tbl %>% slice_sample(n = 25)

mtpl1_testing_20_tbl  <- mtpl1_testing_tbl  %>% slice_sample(n = 25)
```
...
plot_posterior_tbl <- freqmodel_pois_compare_tbl %>%
...
plot_tbl <- plot_posterior_tbl %>%
  select(policy_id) %>%
  distinct() %>%
  slice_sample(n = 20) %>%
  inner_join(plot_posterior_tbl, by = "policy_id")

ggplot(plot_tbl) +
  geom_boxplot(aes(x = policy_id, y = freq_mean, colour = prior_id)) +
  expand_limits(y = 0) +
  labs(x = "Policy ID", y = "Mean Frequency", colour = "Prior") +
  ggtitle("Boxplot of Posterior Distribution of Sample Policies") +
...
plot_tbl <- distrib_comparison_tbl %>%
  ...
  unnest(freqmodel_output) %>%
  semi_join(mtpl1_training_20_tbl, by = "policy_id")
...
barplot_tbl <- plot_tbl %>%
  count(distrib, policy_id, sample_count)

ggplot(barplot_tbl) +
  geom_col(aes(x = sample_count, y = n, fill = distrib),
