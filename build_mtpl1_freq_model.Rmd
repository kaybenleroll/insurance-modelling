...
calculate_priorpred_model <- function(prior_mean, prior_sd,
  ...
  priorpred_freqmodel_stanreg <- stan_glm(
      ...
      prior_PD = TRUE,
      seed     = stan_seed,
      prior_intercept = normal(location = prior_mean, scale = prior_sd, autoscale = autoscale),
      prior           = normal(location = incpt_mean, scale = incpt_sd, autoscale = autoscale)
      )
...
calculate_priorpred_summary <- function(priorpred_stanreg,
                                        priorpred_data_tbl) {
  
  n_sample <- priorpred_data_tbl %>% nrow()
  
  priorpred_summary_stats_tbl <- priorpred_stanreg %>%
    add_linpred_draws(
  ...
  return(priorpred_summary_stats_tbl)
}
...
priorpred_priorparams_tbl <- tribble(
    ~p_m,    ~p_s,    ~i_m,    ~i_s,    ~a_s,
     0.0,     0.5,    -7.0,     0.5,    TRUE,
     0.0,     0.5,    -7.0,     1.0,    TRUE,
     0.0,     0.5,    -7.0,     1.5,    TRUE,
     0.0,     0.5,    -7.0,     2.0,    TRUE,

     0.0,     0.5,    -7.0,     0.5,   FALSE,
     0.0,     0.5,    -7.0,     1.0,   FALSE,
     0.0,     0.5,    -7.0,     1.5,   FALSE,
     0.0,     0.5,    -7.0,     2.0,   FALSE,

     0.0,     1.0,    -7.0,     0.5,    TRUE,
     0.0,     1.0,    -7.0,     1.0,    TRUE,
     0.0,     1.0,    -7.0,     1.5,    TRUE,
     0.0,     1.0,    -7.0,     2.0,    TRUE,

     0.0,     1.0,    -7.0,     0.5,   FALSE,
     0.0,     1.0,    -7.0,     1.0,   FALSE,
     0.0,     1.0,    -7.0,     1.5,   FALSE,
     0.0,     1.0,    -7.0,     2.0,   FALSE,
...
priorpred_summary_tbl <- priorpred_priorparams_tbl %>%
  mutate(
    prior_stanreg = pmap(
        list(prior_mean = p_m,
             prior_sd   = p_s,
    ...
    prior_data = map(prior_stanreg,
                     calculate_priorpred_summary,
                     priorpred_data_tbl = priorpred_data_tbl)
    )

priorpred_summary_tbl %>% glimpse()
```


```{r create_priorpred_mean_boxplots, echo=TRUE}
# priorpred_summary_tbl %>%
#   unnest(prior_data) %>%
#   ggplot(.) +
#     geom_boxplot(aes(x = prior_id, group = prior_id, y = freqmean_mean)) +
#     scale_y_log10()
```
