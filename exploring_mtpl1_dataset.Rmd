...
```{r set_bootstrap_count, echo=TRUE}
n_bootstraps <- 200
```
...
This visualisation could mislead, as it suggests we are using the bootstrap
on each individual value, whereas we are really sampling the joint distribution
across all variables and calculating the joint values of the claim rate. To
fix this, we instead plot each joint set of values.

```{r construct_bootstrap_claimrate_joint_estimates, echo=TRUE}
...
### Calculate Individual Bootstrap Estimates

We now switch our attention to running the bootstrap estimates on a per
quantity basis, sampling each subset of the data rather than the whole
dataset.


```{r calculate_individual_bootstrap_estimates, echo=TRUE}
...
```{r plot_single_bootstrap_values, echo=TRUE}
