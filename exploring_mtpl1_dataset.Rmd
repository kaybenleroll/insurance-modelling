...
claimrate_vardata_tbl %>% pull(data) %>% .[[1]] %>% .$data_plot %>% print()

claimrate_vardata_tbl %>% pull(data) %>% .[[2]] %>% .$data_plot %>% print()

claimrate_vardata_tbl %>% pull(data) %>% .[[3]] %>% .$data_plot %>% print()

claimrate_vardata_tbl %>% pull(data) %>% .[[4]] %>% .$data_plot %>% print()

claimrate_vardata_tbl %>% pull(data) %>% .[[5]] %>% .$data_plot %>% print()

claimrate_vardata_tbl %>% pull(data) %>% .[[6]] %>% .$data_plot %>% print()
```
...
construct_plot <- function(varname, data_tbl) {
  data_plot <- ggplot(data_tbl) +
    geom_boxplot(aes(x = !! sym(varname), y = claim_rate, group = !! sym(varname))) +
    expand_limits(y = 0) +
    xlab(varname) +
    ylab("Claim Rate") +
...
## Explore Region/Density Interaction

We also want to see how the `density` variable in the `region` parameter.
